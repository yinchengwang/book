...
import { updateSudokuBestScore } from "@/utils/storage";
import "./index.scss";
...
class SudokuPage extends Component {
  ...
  handleDifficultyChange = (difficulty: number) => {
    this.stopTimer();
    this.setState(
      { gameState: createInitialState(difficulty) as SudokuState },
      () => {
        this.startTimer();
      },
    );
  ...
  handleNumberSelect = (num: number) => {
    ...
    checkWin(gameState);
    // 如果游戏完成，触发成就引擎并持久化 bestScore
    if (gameState.isGameOver) {
      evaluate({ type: "sudoku.complete", hintCount: gameState.hintCount });
      updateSudokuBestScore(this.calculateStars());
    }
    this.setState({ gameState: { ...gameState } });
  };
  ...
  handleRestart = () => {
    ...
    this.stopTimer();
    this.setState(
      { gameState: createInitialState(gameState.difficulty) as SudokuState },
      () => {
        this.startTimer();
      },
    );
    ...
    return (
      ...
      >
          ...
          {board.map((row, rowIndex) => (
            <View key={rowIndex} className="sudoku-row">
              {row.map((cell, colIndex) => (
                <View
                  key={colIndex}
                  className={`sudoku-cell ${cell.isGiven ? "given" : ""} ${
                    rowIndex === cursorRow && colIndex === cursorCol
                      ? "cursor"
                      : ""
        ...
        <View className="number-pad">
          ...
          <View className="num-btn erase-btn" onClick={this.handleErase}>
            ×
          </View>
        </View>
            ...
            <View className="complete-card">
              ...
              <View className="stars-container">
                {[1, 2, 3].map((star) => (
                  <Text
                    key={star}
                    className={`star ${star <= this.calculateStars() ? "earned" : ""}`}
                  >
                    ★
                  </Text>
                ))}
              ...
              <View className="complete-stats">
                ...
                <View className="stat-row">
                  ...
                  <Text className="stat-value">
                    {SUDOKU_CONFIG.DIFFICULTIES[difficulty].name}
                  </Text>
                </View>
                <View className="stat-row">
