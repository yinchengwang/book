...
import { update2048BestScore } from "@/utils/storage";
import "./index.scss";
...
class Game2048Page extends Component {
  ...
  handleMove = (direction: "left" | "right" | "up" | "down") => {
    ...
    if (!gameState.isGameOver) {
      ...
      // 如果有合并，触发成就引擎
      if (result.mergedTile > 0) {
        evaluate({ type: "game2048.merge", tileValue: result.mergedTile });
      }
      // 持久化 bestScore
      if (gameState.score > gameState.bestScore) {
        update2048BestScore(gameState.score);
      }
  ...
  handleTouchEnd = (e: any) => {
    const { gameState } = this.state;
    if (gameState.isGameOver || (gameState.hasWon && !gameState.keepGoing))
      return;
  ...
  render() {
    const { gameState } = this.state;
    const { board, score, bestScore, isGameOver, hasWon, keepGoing } =
      gameState;
    ...
    return (
      ...
      >
        ...
        <View className="direction-pad">
          <View className="dir-btn up" onClick={() => this.handleMove("up")}>
            ↑
          </View>
          <View
        ...
        <View className="action-btns">
          <View className="action-btn" onClick={this.handleRestart}>
            🔄 新游戏
          </View>
          <View className="action-btn" onClick={this.handleBack}>
            🏠 主页
          </View>
        </View>
        ...
        {hasWon && !keepGoing && (
          <View className="win-overlay">
            ...
            <View className="restart-btn" onClick={this.handleKeepGoing}>
              继续游戏
            </View>
            <View
              className="restart-btn"
              style="margin-top: 20rpx; background: #434343;"
        ...
        {isGameOver && (
          <View className="game-over-overlay">
            ...
            <View className="restart-btn" onClick={this.handleRestart}>
              再来一局
            </View>
            <View
              className="restart-btn"
