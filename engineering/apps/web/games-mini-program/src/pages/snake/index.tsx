...
import { getSnakeBestScore, updateSnakeBestScore } from "@/utils/storage";
    ...
    timerRef.current = setInterval(() => {
      setGameState((prevState) => {
        ...
        if (newState.score > oldScore) {
          evaluate({ type: "snake.eat", score: newState.score });
        }
        // 检测游戏结束
        if (newState.isGameOver) {
          evaluate({ type: "snake.gameOver", score: newState.score });
          // 更新最高分
          updateSnakeBestScore(newState.score);
        }
        return newState;
      });
    }, speed);
    ...
    >
      <View className="header">
        <Text className="score">
          得分: <Text className="score-value">{gameState.score}</Text>
        </Text>
        <Text className="score">
          速度: {Math.round((150 / gameState.speed) * 100)}%
        </Text>
      </View>
      ...
      {gameState.isGameOver && (
        <View className="game-over-overlay">
          ...
          <View className="restart-btn" onClick={startGame}>
            重新开始
          </View>
          <View
