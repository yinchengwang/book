...
export function setSudokuChapterStars(
  chapterId: number,
  levelId: number,
  stars: number,
): boolean {
...
export function setMatch3ChapterStars(
  chapterId: number,
  levelId: number,
  stars: number,
): boolean {
...
export function getAchievements(): AchievementData {
  return getData<AchievementData>("achievements", {
    unlocked: [],
...
export function setAchievements(data: AchievementData): boolean {
  return setData("achievements", data);
}
...
export function migrateData(): boolean {
  try {
    ...
    if (version < 1) {
      ...
      const achievementsData = Taro.getStorageSync(getKey("achievements"));
      if (!achievementsData) {
