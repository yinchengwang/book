...
function wouldMatch(board: Cell[][], row: number, col: number, gem: GemType): boolean {
  // 检查水平
  let hCount = 1
  for (let c = col - 1; c >= 0 && board[row][c] && board[row][c].gem === gem; c--) hCount++
  for (let c = col + 1; c < BOARD_SIZE && board[row][c] && board[row][c].gem === gem; c++) hCount++
  if (hCount >= 3) return true

  // 检查垂直
  let vCount = 1
  for (let r = row - 1; r >= 0 && board[r] && board[r][col] && board[r][col].gem === gem; r--) vCount++
  for (let r = row + 1; r < BOARD_SIZE && board[r] && board[r][col] && board[r][col].gem === gem; r++) vCount++
...
export function findMatches(board: Cell[][]): { cells: Set<string>; type: 'line' | 'lshape' | 'tshape'; direction?: 'h' | 'v' }[] {
  ...
  for (let col = 0; col < BOARD_SIZE; col++) {
    ...
    while (row < BOARD_SIZE) {
      ...
      if (cell.gem !== null && !cell.obstacle) {
        let end = row
        while (end + 1 < BOARD_SIZE && board[end + 1][col].gem === cell.gem && !board[end + 1][col].obstacle) end++
