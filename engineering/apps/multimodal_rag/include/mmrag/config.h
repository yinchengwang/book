...
struct EmbeddingConfig {
    std::string model_path;                     // 模型路径（GGUF 文件路径）
    std::string model_type = "simple";          // 模型类型: simple / kbase / http
    ...
    std::string api_url;                        // API base URL（如 https://open.bigmodel.cn）
    std::string api_key;                        // API Key
    std::string api_model = "embedding-3";      // API 模型名
    int api_dimension = 1024;                   // API 返回向量维度
    int timeout_seconds = 30;                   // HTTP 超时（秒）
};
...
struct HNSWConfig {
    ...
    std::string index_type = "faiss_hnsw";      // 索引类型: "flat_array" | "faiss_hnsw"
};
...
struct ServerConfig {
    ...
    std::string data_dir = "./rag_data";        // 数据目录（上传的文档存放位置）
};
