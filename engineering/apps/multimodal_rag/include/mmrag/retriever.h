...
public:
    ...
    // 设置 chunk 仓库（用于回填检索结果的 chunk 内容）
    void set_chunk_repo(std::shared_ptr<class ChunkRepository> repo) { chunk_repo_ = std::move(repo); }

private:
    std::shared_ptr<VectorIndex> index_;
    std::shared_ptr<EmbeddingService> embed_service_;
    std::shared_ptr<class ChunkRepository> chunk_repo_;
