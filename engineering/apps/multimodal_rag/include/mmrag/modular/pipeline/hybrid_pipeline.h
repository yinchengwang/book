...
#include "mmrag/graph_retriever.h"
...
public:
     ...
     */
    void set_graph_retriever(std::shared_ptr<mmrag::GraphRetriever> retriever);
    ...
    std::shared_ptr<mmrag::GraphRetriever> graph_retriever() const { return graph_retriever_; }
...
private:
     ...
     * @brief 执行 Graph 检索
     */
    std::vector<mmrag::RetrievalResult> retrieve_graph(
        const std::string& query, int top_k);
    ...
    std::vector<mmrag::RetrievalResult> fuse_with_rrf(
        ...
        const std::vector<mmrag::RetrievalResult>& graph_results,
        int top_k);
    ...
    std::shared_ptr<mmrag::GraphRetriever> graph_retriever_; // Graph 知识图谱检索器
