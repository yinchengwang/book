...
#include "mmrag/entity_extractor.h"
...
public:
     ...
     */
    void set_entity_extractor(std::shared_ptr<mmrag::EntityExtractor> extractor);
...
private:
     ...
     * @return 提取的实体列表
     */
    std::vector<mmrag::KGEntity> extract_entities(const std::string& query);
     ...
     * @return 检索结果
     */
    mmrag::GraphRetrievalResult retrieve_graph(
        const std::vector<mmrag::KGEntity>& entities,
        const mmrag::RetrievalConfig& config);
    ...
    std::string build_graph_context(
        const mmrag::KGSubgraph& subgraph,
        const std::vector<mmrag::RetrievedChunk>& chunks);
    ...
    std::shared_ptr<mmrag::EntityExtractor> entity_extractor_;  // 实体提取器
    std::shared_ptr<mmrag::GraphRetriever> graph_retriever_;    // Graph 检索器
    std::shared_ptr<mmrag::HNSWRetriever> hnsw_retriever_;      // HNSW 向量检索器（混合模式）
    mmrag::GraphRetrievalConfig graph_config_;                  // Graph 检索配置
