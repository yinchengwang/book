...
private:
     ...
     * @return 检索结果
     */
    std::vector<mmrag::RetrievalResult> retrieve(
     ...
     */
    mmrag::ReflectionResult evaluate_retrieval_quality(
     ...
     * @return 是否需要纠正
     */
    bool needs_correction(const mmrag::ReflectionResult& evaluation);
     ...
     * @return 评估结果
     */
    mmrag::ReflectionResult parse_evaluation_response(const std::string& response);
    ...
    std::unique_ptr<mmrag::CorrectiveRAG> corrective_rag_;     // Corrective RAG 决策器
    mmrag::SelfRAGConfig self_rag_config_;                     // Self-RAG 配置
