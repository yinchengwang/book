...
#include "mmrag/query_expander.h"
#include "mmrag/reranker.h"
#include "mmrag/adaptive_rrf.h"
...
public:
     ...
     */
    void set_query_expander(std::shared_ptr<mmrag::QueryExpander> expander);

    /**
     * @brief 设置 HNSW 检索器
     * @param retriever HNSW 检索器
     */
    void set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever);

    /**
     * @brief 设置 BM25 检索器
     * @param retriever BM25 检索器
     */
    void set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever);

    /**
     * @brief 设置重排序器
     * @param reranker 重排序器
     */
    void set_reranker(std::shared_ptr<mmrag::Reranker> reranker);
...
private:
     ...
     * @brief 执行向量检索
     */
    std::vector<mmrag::RetrievalResult> retrieve_vectors(
        const std::string& query, int top_k);

    /**
     * @brief 执行 BM25 检索
     */
    std::vector<mmrag::RetrievalResult> retrieve_bm25(
        const std::string& query, int top_k);

    /**
     * @brief 执行 RRF 融合
     */
    std::vector<mmrag::RetrievalResult> fuse_with_rrf(
     ...
     * @brief 执行重排序
     */
    std::vector<mmrag::RetrievalResult> rerank_results(
        const std::string& query,
        const std::vector<mmrag::RetrievalResult>& candidates,
        int top_n);

    std::shared_ptr<mmrag::QueryExpander> query_expander_;   // 查询扩展器
    std::shared_ptr<mmrag::HNSWRetriever> hnsw_retriever_;   // HNSW 向量检索器
    std::shared_ptr<mmrag::BM25Retriever> bm25_retriever_;  // BM25 全文检索器
    std::shared_ptr<mmrag::Reranker> reranker_;             // 重排序器
    std::unique_ptr<mmrag::AdaptiveRRF> adaptive_rrf_;       // 自适应 RRF 融合器
