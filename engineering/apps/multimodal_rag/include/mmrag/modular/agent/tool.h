...
public:
     ...
     * @param top_k 返回结果数量
     */
    VectorSearchTool(std::shared_ptr<mmrag::Retriever> retriever, int top_k = 5);
...
private:
    std::shared_ptr<mmrag::Retriever> retriever_;
    int top_k_;
};
...
public:
     ...
     * @param top_k 返回结果数量
     */
    BM25SearchTool(std::shared_ptr<mmrag::Retriever> retriever, int top_k = 5);
...
public:
     ...
     * @param top_k 返回结果数量
     */
    GraphSearchTool(std::shared_ptr<mmrag::GraphRetriever> graph_retriever, int top_k = 5);
...
private:
    std::shared_ptr<mmrag::GraphRetriever> graph_retriever_;
    int top_k_;
};
...
public:
     ...
     */
    LLMGenerateTool(std::shared_ptr<mmrag::LLMService> llm_service);
...
private:
    std::shared_ptr<mmrag::LLMService> llm_service_;
};
