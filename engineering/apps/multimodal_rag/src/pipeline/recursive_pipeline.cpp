...
void RecursivePipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void RecursivePipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
...
std::vector<mmrag::RetrievalResult> RecursivePipeline::merge_results(
    const std::vector<SubQuery>& sub_queries) {
    std::vector<mmrag::RetrievalResult> merged;
...
std::vector<mmrag::RetrievalResult> RecursivePipeline::retrieve(
