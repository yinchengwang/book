...
AdvancedPipeline::AdvancedPipeline()
    : initialized_(false),
      adaptive_rrf_(std::make_unique<mmrag::AdaptiveRRF>()) {}
...
bool AdvancedPipeline::init(const ModularConfig& config) {
    ...
    // 初始化 LLM 服务 — 对 Ollama 类型无需 model_path，模型由 LLM 服务自身配置
    // 只有当没有外部注入 llm_ 时才自行创建
    if (!llm_) {
        llm_ = mmrag::create_llm_service();
        if (llm_) {
            RAG_INFO("LLM 服务创建成功: " + llm_->model_path());
        } else {
            RAG_WARN("LLM 服务创建失败，高级 pipeline 将无法生成回答");
        }
    }
...
ModularQueryResult AdvancedPipeline::query(const ModularQuery& query) {
    ...
    std::vector<mmrag::RetrievalResult> all_hnsw_results;
    std::vector<mmrag::RetrievalResult> all_bm25_results;
...
void AdvancedPipeline::set_query_expander(std::shared_ptr<mmrag::QueryExpander> expander) {
    query_expander_ = expander;
}

void AdvancedPipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void AdvancedPipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
    bm25_retriever_ = retriever;
}

void AdvancedPipeline::set_reranker(std::shared_ptr<mmrag::Reranker> reranker) {
    reranker_ = reranker;
}
...
std::vector<mmrag::RetrievalResult> AdvancedPipeline::retrieve_vectors(
...
std::vector<mmrag::RetrievalResult> AdvancedPipeline::retrieve_bm25(
    const std::string& query, int top_k) {
    if (!bm25_retriever_) {
        return {};
    }
...
std::vector<mmrag::RetrievalResult> AdvancedPipeline::fuse_with_rrf(
    const std::vector<mmrag::RetrievalResult>& hnsw_results,
    const std::vector<mmrag::RetrievalResult>& bm25_results,
    int top_k) {
...
std::vector<mmrag::RetrievalResult> AdvancedPipeline::rerank_results(
    const std::string& query,
    const std::vector<mmrag::RetrievalResult>& candidates,
    int top_n) {
    ...
    try {
        // 转换为 Chunk 列表
        std::vector<mmrag::Chunk> chunks;
        std::unordered_map<std::string, mmrag::Chunk> id_to_chunk;
        ...
        // 构建重排后的结果
        std::vector<mmrag::RetrievalResult> results;
