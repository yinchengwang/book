...
ModularQueryResult GraphPipeline::query(const ModularQuery& query) {
    ...
    mmrag::RetrievalConfig retrieval_config;
    retrieval_config.top_k = top_k;

    mmrag::GraphRetrievalResult graph_result;
    std::vector<mmrag::RetrievalResult> vector_results;
...
void GraphPipeline::set_entity_extractor(std::shared_ptr<mmrag::EntityExtractor> extractor) {
    entity_extractor_ = extractor;
}

void GraphPipeline::set_graph_retriever(std::shared_ptr<mmrag::GraphRetriever> retriever) {
    graph_retriever_ = retriever;
}

void GraphPipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

std::vector<mmrag::KGEntity> GraphPipeline::extract_entities(const std::string& query) {
...
mmrag::GraphRetrievalResult GraphPipeline::retrieve_graph(
...
std::vector<mmrag::RetrievalResult> GraphPipeline::retrieve_vectors(
...
std::string GraphPipeline::build_graph_context(
    ...
    if (!subgraph.entities.empty()) {
        ...
        for (const auto& entity : subgraph.entities) {
            oss << "- " << entity.name;
            if (entity.type != mmrag::EntityType::UNKNOWN) {
