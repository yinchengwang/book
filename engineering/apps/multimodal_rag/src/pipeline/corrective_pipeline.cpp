...
CorrectivePipeline::CorrectivePipeline()
    ...
    self_rag_config_.evaluation_mode = mmrag::SelfRAGConfig::EvaluationMode::LLM_JUDGE;
}
...
bool CorrectivePipeline::init(const ModularConfig& config) {
    ...
    // 初始化 CorrectiveRAG
    corrective_rag_ = mmrag::create_corrective_rag(self_rag_config_);
...
ModularQueryResult CorrectivePipeline::query(const ModularQuery& query) {
    ...
    std::vector<mmrag::RetrievalResult> all_retrieval_results;
    ...
    std::sort(result.context.begin(), result.context.end(),
              [](const mmrag::RetrievalResult& a, const mmrag::RetrievalResult& b) {
                  return a.score > b.score;
              });

    std::vector<mmrag::RetrievalResult> unique_results;
...
void CorrectivePipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void CorrectivePipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
    bm25_retriever_ = retriever;
}

std::vector<mmrag::RetrievalResult> CorrectivePipeline::retrieve(
...
mmrag::ReflectionResult CorrectivePipeline::evaluate_retrieval_quality(
    const std::string& query,
    const std::vector<mmrag::RetrievalResult>& results) {
    if (results.empty()) {
        mmrag::ReflectionResult empty_result;
        return empty_result;
    }
    ...
    if (!llm_ || !llm_->is_loaded()) {
        RAG_ERROR("LLM 服务不可用");
        mmrag::ReflectionResult error_result;
        return error_result;
    }
    ...
    mmrag::ReflectionResult fallback_result;
    return fallback_result;
}

bool CorrectivePipeline::needs_correction(const mmrag::ReflectionResult& evaluation) {
    // 如果评估分数低于阈值，需要纠正
...
std::string CorrectivePipeline::perform_correction(
    const std::string& query,
    const mmrag::ReflectionResult& evaluation,
    int iteration) {
    // 构建纠正提示词
...
mmrag::ReflectionResult CorrectivePipeline::parse_evaluation_response(
    const std::string& response) {
    mmrag::ReflectionResult result;
