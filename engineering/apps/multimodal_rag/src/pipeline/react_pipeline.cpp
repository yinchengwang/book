...
#include "mmrag/modular/agent/agent.h"
#include "mmrag/modular/agent/tool.h"
...
bool ReActPipeline::init(const ModularConfig& config) {
    ...
    mmrag::modular::agent::AgentConfig agent_config;
    ...
    agent_ = std::make_unique<mmrag::modular::agent::Agent>(agent_config);
    ...
    // 注册 Tools
    if (hnsw_retriever_) {
        auto vector_tool = std::make_shared<mmrag::modular::agent::VectorSearchTool>(
            hnsw_retriever_, config.retrieval.top_k);
        agent_->register_tool(vector_tool);
    }

    if (bm25_retriever_) {
        auto bm25_tool = std::make_shared<mmrag::modular::agent::BM25SearchTool>(
...
void ReActPipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void ReActPipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
...
std::vector<mmrag::RetrievalResult> ReActPipeline::retrieve(
