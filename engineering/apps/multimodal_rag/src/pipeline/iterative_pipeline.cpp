...
ModularQueryResult IterativePipeline::query(const ModularQuery& query) {
    ...
    std::vector<mmrag::RetrievalResult> all_results;
...
void IterativePipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void IterativePipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
    bm25_retriever_ = retriever;
}

std::vector<mmrag::RetrievalResult> IterativePipeline::retrieve(
