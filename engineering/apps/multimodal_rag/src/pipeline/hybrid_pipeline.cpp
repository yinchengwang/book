...
void HybridPipeline::set_hnsw_retriever(std::shared_ptr<mmrag::HNSWRetriever> retriever) {
    hnsw_retriever_ = retriever;
}

void HybridPipeline::set_bm25_retriever(std::shared_ptr<mmrag::BM25Retriever> retriever) {
    bm25_retriever_ = retriever;
}

void HybridPipeline::set_graph_retriever(std::shared_ptr<mmrag::GraphRetriever> retriever) {
    graph_retriever_ = retriever;
}

std::vector<mmrag::RetrievalResult> HybridPipeline::retrieve_vectors(
...
std::vector<mmrag::RetrievalResult> HybridPipeline::retrieve_bm25(
    const std::string& query, int top_k) {
    if (!bm25_retriever_) {
        return {};
    }
...
std::vector<mmrag::RetrievalResult> HybridPipeline::retrieve_graph(
    const std::string& query, int top_k) {
    if (!graph_retriever_) {
        return {};
    }
...
std::vector<mmrag::RetrievalResult> HybridPipeline::fuse_with_rrf(
