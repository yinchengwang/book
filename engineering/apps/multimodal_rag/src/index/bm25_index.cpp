...
void BM25Index::add_internal(const std::string& id, const std::string& content) {
...
std::vector<std::pair<std::string, float>> BM25Index::search(
    ...
    for (int i = 0; i < doc_count_; ++i) {
        // 跳过 tombstone
        if (i < static_cast<int>(doc_ids_.size()) && tombstone_count_ > 0) {
            // 检查该位置是否是 tombstone
            const std::string& id = doc_ids_[i];
            // 这里简化：tombstone 会从 documents_ 中移除，所以检查 documents_ 即可
            if (documents_.find(id) == documents_.end()) {
                continue;
            }
        }
    ...
    // 排序
    std::partial_sort(scores.begin(), scores.begin() + std::min(top_k, static_cast<int>(scores.size())), scores.end(),
...
void BM25Index::remove_internal(const std::string& id) {
    ...
    // 检查是否需要 consolidate
    if (tombstone_count_ > 0 && doc_count_ > 0 &&
        static_cast<double>(tombstone_count_) / doc_count_ > 0.1) {
        consolidate();
    }
}
...
size_t BM25Index::consolidate() {
    // 清理 tombstone 的文档
    size_t cleaned = 0;
    for (auto it = doc_ids_.begin(); it != doc_ids_.end(); ) {
        if (documents_.find(*it) == documents_.end()) {
            // 这是 tombstone，清理掉
            size_t idx = std::distance(doc_ids_.begin(), it);
    ...
    doc_count_ -= static_cast<int>(cleaned);
    tombstone_count_ = 0;
    update_stats();

    RAG_INFO("BM25 consolidate: cleaned " + std::to_string(cleaned) + " tombstones");
    return cleaned;
}
