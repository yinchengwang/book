...
Config Config::default_config() {
    ...
    // Embedding
    // Phase 3: 默认使用 kbase 真实 GGUF MiniLM-L6 推理。
    // 当 model_path 不可用或 model_type != "kbase" 时降级到 SimpleEmbeddingService。
    config.embedding = {
        .model_path = "./reference/models/minilm-l6-q4_k_m.gguf",
        .model_type = "kbase",
        .batch_size = 32,
        .max_length = 512,
        .device = "cpu",
    ...
    // HNSW
    // Phase 3: dim 默认改为 384 以匹配 MiniLM-L6 输出维度。
    config.hnsw = {
        .dim = 384,
...
Config ConfigLoader::load_from_string(const std::string& yaml_content) {
    ...
    while (std::getline(iss, line)) {
        ...
        // Strip inline comments (# ...) but keep # inside quoted strings
        ...
        // 解析顶层和嵌套配置
        // 支持嵌套 key 如 "embedding.type"、"embedding.api_url"
        if (key == "version") {
            config.version = value;
        } else if (key == "data_dir") {
            config.data_dir = value;
            config.server.data_dir = value;  // 同步到 server config
        } else if (key == "index_dir") {
            config.index_dir = value;
        } else if (key == "embedding.type") {
            config.embedding.model_type = value;
        } else if (key == "embedding.dim" || key == "hnsw.dim") {
            config.hnsw.dim = std::stoi(value);
            config.embedding.api_dimension = std::stoi(value);
        } else if (key == "embedding.model" || key == "embedding.model_path") {
            config.embedding.model_path = value;
        } else if (key == "embedding.api_url") {
            config.embedding.api_url = value;
        } else if (key == "embedding.api_key") {
            config.embedding.api_key = value;
        } else if (key == "embedding.api_model") {
            config.embedding.api_model = value;
        } else if (key == "embedding.timeout") {
            config.embedding.timeout_seconds = std::stoi(value);
...
void ConfigLoader::apply_env_overrides(Config& config) {
    ...
    if (const char* env = std::getenv("RAG_DATA_DIR")) {
        config.data_dir = env;
        config.server.data_dir = env;  // 同步到 server config
        config.index_dir = std::string(env) + "/index";
        config.cache_dir = std::string(env) + "/cache";
    }
    if (const char* env = std::getenv("MMRAG_DATA_DIR")) {
        config.data_dir = env;
        config.server.data_dir = env;  // 同步到 server config
        config.index_dir = std::string(env) + "/index";
        config.cache_dir = std::string(env) + "/cache";
