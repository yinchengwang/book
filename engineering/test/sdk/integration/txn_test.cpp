...
class TxnTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        ...
        /* 创建测试表 */
        ASSERT_EQ(mmdb_sqlite_exec(db_->db,
            "CREATE TABLE test_txn (id INTEGER PRIMARY KEY, value TEXT)"), 0);
    }
...
TEST_F(TxnTest, BeginCommit) {
    ...
    /* 插入数据 */
    ASSERT_EQ(mmdb_sqlite_exec(db_->db,
        "INSERT INTO test_txn (id, value) VALUES (1, 'hello')"), 0);
    ...
    ASSERT_EQ(sqlite3_prepare_v2(db_->db,
        "SELECT value FROM test_txn WHERE id = 1", -1, &stmt, nullptr), SQLITE_ROW);
...
TEST_F(TxnTest, BeginAbort) {
    ...
    sqlite3_stmt* stmt = nullptr;
    ASSERT_EQ(sqlite3_prepare_v2(db_->db,
        "SELECT COUNT(*) FROM test_txn", -1, &stmt, nullptr), SQLITE_ROW);
    ASSERT_EQ(sqlite3_step(stmt), SQLITE_ROW);
...
TEST_F(TxnTest, NestedSavepoint) {
    ...
    /* 创建 Savepoint */
    ASSERT_EQ(mmdb_sqlite_exec(db_->db, "SAVEPOINT sp1"), 0);

    /* 在 Savepoint 内插入数据 */
    ASSERT_EQ(mmdb_sqlite_exec(db_->db,
        "INSERT INTO test_txn (id, value) VALUES (2, 'world')"), 0);

    /* 回滚到 Savepoint */
    ASSERT_EQ(mmdb_sqlite_exec(db_->db, "ROLLBACK TO SAVEPOINT sp1"), 0);
