...
TEST(VectorIndexTypeTest, ConstantsDefined) {
    EXPECT_EQ(VECTOR_INDEX_NONE, 0);
    EXPECT_EQ(VECTOR_INDEX_HNSW, 1);
    EXPECT_EQ(VECTOR_INDEX_IVF_PQ, 2);
}
...
TEST(VectorIndexTypeTest, StructHasActiveIndexType) {
    ...
    EXPECT_EQ(db.active_index_type, VECTOR_INDEX_NONE);

    db.active_index_type = VECTOR_INDEX_HNSW;
    EXPECT_EQ(db.active_index_type, VECTOR_INDEX_HNSW);

    db.active_index_type = VECTOR_INDEX_IVF_PQ;
    EXPECT_EQ(db.active_index_type, VECTOR_INDEX_IVF_PQ);
}