...
int main(int argc, char *argv[]) {
    ...
    printf("=== Ball Tree SIFT Recall Test ===\n\n");
    printf("Dataset: %s\n\n", data_dir);
    ...
    /* 创建 Ball Tree 索引 */
    printf("Building Ball Tree index (dims=%d)...\n", dims);
    ball_tree_t *idx = ball_tree_create(dims);
    if (!idx) {
        fprintf(stderr, "Failed to create Ball Tree index\n");
        free(ids);
        free(base);
        free(query);
        free(gt);
    ...
    /* 构建索引 */
    int ret = ball_tree_build(idx, n_base, base, ids);
    if (ret != 0) {
        fprintf(stderr, "Failed to build Ball Tree index: %d\n", ret);
    ...
    ball_tree_stats(idx, &n_vectors, &n_nodes);
    printf("Index built: %d vectors, %d nodes\n\n", n_vectors, n_nodes);

    /* Ball Tree 是精确搜索，测试 Recall@1, Recall@10, Recall@50 */
    printf("Testing recall (exact search)...\n");
    ...
    for (int tk = 0; tk < 3; tk++) {
        ...
        for (int q = 0; q < n_query; q++) {
            float dists[100];
            int32_t result_ids[100];
            memset(result_ids, -1, sizeof(result_ids));
            memset(dists, 0, sizeof(dists));

            int n_found = ball_tree_search(idx, &query[q * dims], k, result_ids, dists);

            float recall = compute_recall_at_k(result_ids, &gt[q * gt_k], k);
            total_recall += recall;
        }
