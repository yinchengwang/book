...
int main(int argc, char *argv[]) {
    ...
    printf("=== FAISS IVF Compat SIFT Recall Test ===\n\n");
    printf("Dataset: %s\n\n", data_dir);
    ...
    int nlist = 100;
    printf("Building FAISS IVF compat index (nlist=%d)...\n", nlist);

    faiss_ivf_compat_t *idx = faiss_ivf_compat_create(dims, nlist);
    if (!idx) {
        fprintf(stderr, "Error: failed to create FAISS IVF compat index\n");
        free(base); free(query); free(gt);
        return 1;
    }

    int ret = faiss_ivf_compat_train(idx, n_base, base);
    if (ret != 0) {
    ...
    faiss_ivf_compat_add(idx, n_base, base, ids);
    free(ids);
    ...
    for (int np = 0; np < 3; np++) {
        ...
        for (int tk = 0; tk < nk; tk++) {
            ...
            for (int q = 0; q < n_query; q++) {
                ...
                faiss_ivf_compat_search(idx, &query[q * dims], k, ids_out, dists, nprobe);
