...
struct RelationData {
    ...
    /* 并发控制 - 锁上下文 */
    lock_manager_t *rd_lockmgr;     /**< 锁管理器 */
    lock_ctx_t   *rd_lockctx;       /**< 锁上下文 */
    bool        rd_explicit_lock;   /**< 是否由调用者显式管理锁 */
};
...
/**
 * @brief 获取 Relation 的表锁
 * @param rel Relation
 * @param mode 锁模式 (LOCK_MODE_S/LOCK_MODE_X)
...
/**
 * @brief 释放 Relation 的表锁
 * @param rel Relation
 */
void relation_unlock(Relation rel);

/**
 * @brief 设置 Relation 的锁管理器
 * @param rel Relation
 * @param mgr 锁管理器（NULL 使用全局锁管理器）
 */
void relation_set_lockmgr(Relation rel, lock_manager_t *mgr);

/**
 * @brief 获取 Relation 的锁管理器
 * @param rel Relation
 * @return 锁管理器指针
 */
