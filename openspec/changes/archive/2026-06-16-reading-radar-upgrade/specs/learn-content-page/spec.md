...
**Reason**: 已由自由格式 Markdown 内容替代，不再强制四段式结构

**Migration**: 每个知识点的 MD 文件自由组织章节

### Requirement: 数据文件按技术栈分割

**Reason**: 已由 `data/learn-deep/{stack}/{quadrant}/{id}.md` 的 MD 文件结构替代 JS 全局变量模式

**Migration**: `data/learn-content-<catId>.js` 文件不再使用
