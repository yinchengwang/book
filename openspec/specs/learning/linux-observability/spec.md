...
为 Linux 可观测性领域提供深度技术文章，涵盖 procfs、CPU/内存/磁盘/网络性能诊断、系统监控工具（htop/sar/perf/blktrace）、eBPF 可观测性、火焰图分析及 NUMA 观测等核心知识点，每篇文章遵循 8 段式模版，强调工具命令示例和排查决策树。

## Requirements

### Requirement: Linux 可观测性深度文章

Linux 可观测性的每篇深度文章 SHALL 覆盖以下知识点（预计 ~14 篇）：
...
| `linux-ebpf-intro` | eBPF 可观测性 | advanced |
